...
fn animate_static_tiles(
    ...
    while pump.accumulator >= pump.interval {
        ...
        // Called for its side effects on the wait frame and the prompt
        // cursor; its own "did anything change" answer no longer decides
        // whether we redraw, because the barber pole below always does.
        let _ = advance_visual_wait_frame(&mut visual.state, &mut prompt_cursor_visible);
        visual.prompt_cursor_visible = prompt_cursor_visible;
        // The live input line's barber pole scrolls one phase per pump
        // tick whether or not a line prompt is open, so every pump tick
        // is a redraw.
