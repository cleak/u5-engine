...
pub(crate) fn combat_monster_attack_result_message(
    ...
    // entire damage and narration chain ... no result line". The two
    // rejecting outcomes fall through to ordinary resolution and narrate
    ...
    if matches!(
        ...
        return Some(format!("{target_name} slept!"));
    }
    if matches!(
