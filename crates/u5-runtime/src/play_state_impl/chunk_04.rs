...
impl PlayState {
    ...
    /// `conversation.md §7.3`: the player pressed the key a `0x83` PAUSE
    /// or `0x8F` WAIT-KEY was blocked on. Resume the stopped response.
    ///
    /// Returns the text the resumed run printed and whether the
    /// conversation ended, matching
    /// [`Self::submit_active_conversation_keyword`]. The keyword path's
    /// prompt echo, Bye shortcut and join bookkeeping have no part here:
    /// nothing was typed, and the response is simply continuing.
    pub fn resume_active_conversation_page(&mut self) -> (String, bool) {
        let avatar_name = self
            .party_names
            .first()
            .map(|name| {
        ...
        let mut ended = false;
        if let Some(session) = self.active_conversation.as_mut() {
            let output = session.resume_after_page_key(&ctx);
    ...
    /// `conversation.md §7.3`: the conversation is blocked on one
    /// keystroke inside a response.
    pub fn active_conversation_awaiting_page_key(&self) -> bool {
        self.active_conversation
            .as_ref()
            .is_some_and(|session| session.awaiting_page_key())
    }
