...
impl ActiveShopSession {
    ...
    // `shops.md §8`: an active shop prints through the ordinary message
    // window's transcript, so there is no modal summary. `modal_summary`
    // and `modal_text` used to compose one - the shop's label on its own
    // row and a per-state key list such as `Cure (C), Heal (H), Resurrect
    // (R), or Space.` - and both shells painted it over the window.
    // `qa/tools/paired_compare.py` read those two rows above otherwise
    // exact healer text (`cleak/u5-engine#21`); every literal in them was
    // the engine's own.
}
