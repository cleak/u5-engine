...
pub fn load_dungeon_wind_tile_entries(
    game_dir: &Path,
) -> io::Result<Option<Vec<DungeonWindTileEntry>>> {
    let path = game_dir.join(DUNGEON_WIND_TILE_TABLE_FILE);
    ...
    parse_dungeon_wind_tile_entries(&text).map(Some)
}

pub fn parse_dungeon_wind_tile_entries(text: &str) -> io::Result<Vec<DungeonWindTileEntry>> {
    let mut entries = Vec::new();
    for (line_index, line) in text.lines().enumerate() {
        let line_number = line_index + 1;
        let line = line
        ...
        if !(4..=5).contains(&parts.len()) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} must be: DUNGEON LEVEL X Y [CELL]"
                ),
            ));
        }
        ...
        let scene = match PlayTarget::from_key(parts[0]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} has invalid dungeon `{}`: {err}",
                    parts[0]
                ),
            )
        })? {
            ...
            _ => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
                        "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} source must be a dungeon"
                    ),
                ));
            }
        };
        let level = parse_u8_literal(parts[1]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} has invalid level `{}`: {err}",
                    parts[1]
                ),
            )
        })?;
        if level >= DUNGEON_SIDE as u8 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} level must be inside 0..7, got {level}"
                ),
            ));
        }
        let x = parse_u8_literal(parts[2]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} has invalid X `{}`: {err}",
                    parts[2]
                ),
            )
        })? as usize;
        let y = parse_u8_literal(parts[3]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} has invalid Y `{}`: {err}",
                    parts[3]
                ),
            )
        })? as usize;
        if x >= DUNGEON_SIDE || y >= DUNGEON_SIDE {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} coordinate must be inside 0..7, got ({x}, {y})"
                ),
            ));
        }
        let expected_cell = if let Some(cell) = parts.get(4) {
            Some(parse_u8_literal(cell).map_err(|err| {
                io::Error::new(
                    err.kind(),
                    format!(
                        "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} has invalid cell `{cell}`: {err}"
                    ),
                )
            })?)
        } else {
        ...
        if entries.iter().any(|entry: &DungeonWindTileEntry| {
            entry.scene == scene && entry.level == level && entry.x == x && entry.y == y
        }) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_WIND_TILE_TABLE_FILE} line {line_number} duplicates {} level {level} at ({x}, {y})",
                    scene.key()
                ),
            ));
        }
        entries.push(DungeonWindTileEntry {
            scene,
            level,
            x,
            y,
...
pub fn load_dungeon_door_entries(game_dir: &Path) -> io::Result<Option<Vec<DungeonDoorEntry>>> {
    let path = game_dir.join(DUNGEON_DOOR_TABLE_FILE);
    let text = match fs::read_to_string(&path) {
        Ok(text) => text,
        Err(err) if err.kind() == io::ErrorKind::NotFound => return Ok(None),
        Err(err) => {
    ...
    parse_dungeon_door_entries(&text).map(Some)
}

pub fn parse_dungeon_door_entries(text: &str) -> io::Result<Vec<DungeonDoorEntry>> {
    let mut entries = Vec::new();
    for (line_index, line) in text.lines().enumerate() {
        let line_number = line_index + 1;
        let line = line
        ...
        if !(5..=6).contains(&parts.len()) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} must be: DUNGEON LEVEL X Y OPEN_CELL [CLOSED_CELL]"
                ),
            ));
        }
        ...
        let scene = match PlayTarget::from_key(parts[0]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} has invalid dungeon `{}`: {err}",
                    parts[0]
                ),
            )
        })? {
            ...
            _ => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
                        "{DUNGEON_DOOR_TABLE_FILE} line {line_number} source must be a dungeon"
                    ),
                ));
            }
        };
        let level = parse_u8_literal(parts[1]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} has invalid level `{}`: {err}",
                    parts[1]
                ),
            )
        })?;
        if level >= DUNGEON_SIDE as u8 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} level must be inside 0..7, got {level}"
                ),
            ));
        }
        let x = parse_u8_literal(parts[2]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} has invalid X `{}`: {err}",
                    parts[2]
                ),
            )
        })? as usize;
        let y = parse_u8_literal(parts[3]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} has invalid Y `{}`: {err}",
                    parts[3]
                ),
            )
        })? as usize;
        if x >= DUNGEON_SIDE || y >= DUNGEON_SIDE {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} coordinate must be inside 0..7, got ({x}, {y})"
                ),
            ));
        }
        let open_cell = parse_u8_literal(parts[4]).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} has invalid open cell `{}`: {err}",
                    parts[4]
                ),
            )
        })?;
        let expected_cell = if let Some(cell) = parts.get(5) {
            let cell = parse_u8_literal(cell).map_err(|err| {
                io::Error::new(
                    err.kind(),
                    format!(
                        "{DUNGEON_DOOR_TABLE_FILE} line {line_number} has invalid closed cell `{cell}`: {err}"
                    ),
                )
            })?;
            if cell == open_cell {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
                        "{DUNGEON_DOOR_TABLE_FILE} line {line_number} open and closed cells must differ"
                    ),
                ));
            }
            Some(cell)
        ...
        if entries.iter().any(|entry: &DungeonDoorEntry| {
            entry.scene == scene && entry.level == level && entry.x == x && entry.y == y
        }) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "{DUNGEON_DOOR_TABLE_FILE} line {line_number} duplicates {} level {level} at ({x}, {y})",
                    scene.key()
                ),
            ));
        }
        entries.push(DungeonDoorEntry {
            scene,
            level,
            x,
            y,
