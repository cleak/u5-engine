    ...
    fn open_dungeon_record() -> Vec<u8> {
        vec![0; DUNGEON_RECORD_LEN]
    }

    fn open_world_grid() -> Vec<u8> {
        vec![5; WORLD_CELLS]
    }

    fn synthetic_tile_atlas(depth: TileGraphicsDepth) -> TileAtlas {
        ...
        for tile in 0..TILE_ATLAS_TILE_COUNT {
            pixels
                .extend(std::iter::repeat((tile as u8) % pixel_limit).take(TILE_ATLAS_TILE_PIXELS));
        }
    ...
    fn test_state(grid: Vec<u8>, x: usize, y: usize) -> PlayState {
    ...
    fn dungeon_state(grid: Vec<u8>, level: u8, x: usize, y: usize) -> PlayState {
