...
fn dungeon_get_refuses_chest_room_trigger_or_unrelated_cell_without_turn() {
    ...
    // `commands.md §5.3`: the `Get` row itself comes from the dungeon arm of
    // the shared echo table, so the handler contributes only what follows it
    // (measured `qa/paired/dungeon-chest-refusals.tsv`).
    ...
    assert_eq!(state.turn, 0);
    // `commands.md §5.8`: "Get on a non-chest prints `Get\nNot here!\n`", the
    // `Get` row coming from the echo table.
    assert_eq!(state.message, DUNGEON_CHEST_GET_NOT_HERE);
...
fn town_talk_reserved_guard_dialog_opens_default_tribute_demand() {
    ...
    assert_eq!(state.message, "\nA guard demands a 10 gp tribute to Blackthorn!\n\nDost thou pay?\n\n:");
    assert_eq!(state.turn, 1);
    assert!(matches!(
...
/// `conversation.md §7.3`: `0x8F` WAIT-KEY "block[s] for one keystroke",
/// and `0x83` PAUSE waits too after redrawing the world view. Both are
/// there to "chunk long responses into reader-friendly pages", so the
/// second page must not print until the player presses something.
#[test]
fn active_conversation_wait_key_pages_the_response() {
    let mut dialogue: HashMap<u16, Vec<String>> = HashMap::new();
    dialogue.insert(
        0x10,
        vec![
    ...
    let enc = |s: &str| s.bytes().map(|b| b ^ 0x80).collect::<Vec<u8>>();
    let mut tale = enc("First page.");
    tale.push(TLK_CODE_WAIT_KEY);
    tale.extend(enc("Second page."));
    tale.push(TLK_CODE_END_OF_RESPONSE);
    let mut raw: HashMap<u16, Vec<Vec<u8>>> = HashMap::new();
    raw.insert(
        0x10,
        vec![
    ...
    handle_play_key_input(&mut state, 'T', "ALE", Path::new("")).unwrap();
    assert!(transcript_has(&state, "First page."));
    // The wait is what this issue is about: the rest of the response has
    // not printed, and the codes carry no prompt of their own.
    assert!(!transcript_has(&state, "Second page."));
    assert!(state.active_conversation_awaiting_page_key());

    // Any key resumes the same field just past the control byte.
