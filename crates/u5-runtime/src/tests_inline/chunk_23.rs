...
fn combat_input_dispatch_routes_play_keys_to_combat_parser() {
    ...
    // Stream re-baselined by `RETRACTIONS.md` R311: the lazily drawn
    // cardinal fallback changes which roll the Giant Rat's reply takes.
    // R308's prologue world tick draws nothing on this engine's
    // presentation-only tick, so it is not part of the shift.
...
fn combat_input_dispatch_reports_weapon_kill_and_keeps_victory_cleanup_live() {
    ...
    // `combat.md §7`/`§14` (`RETRACTIONS.md` R289): the round loop
    // prints the resident `VICTORY!` string - one leading and one
    // trailing newline, one-shot - once the post-action recount finds
    // no hostile left, and then keeps running.
    assert_eq!(
        state.message,
        format!("East\nGiant Rat killed!\n{COMBAT_VICTORY_LINE}")
    );
