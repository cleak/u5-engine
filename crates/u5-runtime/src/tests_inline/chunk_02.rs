...
fn direction_keys_leave_command_letters_for_dispatch() {
    ...
    assert_eq!(
        Direction::from_play_key(char::from(INPUT_CODE_NORTH)),
        Some(Direction::North)
    );
    assert_eq!(
        Direction::from_play_key(char::from(INPUT_CODE_SOUTHWEST)),
        Some(Direction::SouthWest)
    );
    assert_eq!(
        Direction::from_play_key(char::from(INPUT_CODE_NORTHEAST)),
...
fn chargen_pair_records_follow_public_question_table() {
    assert_eq!(
        chargen_question_record_for_pair(ShrineVirtue::Honesty, ShrineVirtue::Compassion).unwrap(),
        2
    );
    assert_eq!(
        chargen_question_record_for_pair(ShrineVirtue::Humility, ShrineVirtue::Honesty).unwrap(),
        8
    );
    assert_eq!(
        chargen_question_record_for_pair(ShrineVirtue::Justice, ShrineVirtue::Sacrifice).unwrap(),
        20
    );
    assert_eq!(
...
fn chargen_application_customizes_only_avatar_identity_and_stats() {
    ...
    assert_eq!(
        save[record + SAVE_CHARACTER_GENDER_OFFSET],
    ...
    assert_eq!(
        u16_at(&save, record + SAVE_CHARACTER_EXPERIENCE_OFFSET),
        150
    );
...
fn u4_transfer_application_maps_avatar_fields_and_preserves_seed_bytes() {
    ...
    let record = SAVE_ROSTER_OFFSET;
    assert_eq!(
        &save[record..record + SAVE_CHARACTER_NAME_LEN],
        b"IOLO1234\0"
    );
    assert_eq!(
        save[record + SAVE_CHARACTER_GENDER_OFFSET],
