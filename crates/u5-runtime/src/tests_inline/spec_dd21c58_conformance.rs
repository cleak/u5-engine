...
fn a_landed_gremlin_attack_steals_food_instead_of_dealing_damage() {
    ...
    //
    // `fix/combat-transcript-2` moved that from a string append onto the
    // arena's own printer, so the row is asserted where it is now drawn.
    // The `Pass` print leaves the cursor mid-row, so this print's leading
    // newline closes that row rather than blanking one
    // (`text-output.md {S}10.4`).
    let mut rows = combat_ai_turn_state(6, 5);
    rows.combat_active = true;
    rows.emit_combat_print("Pass");
    rows.emit_combat_print(&format!(
        "\n{}\n",
        crate::input_dispatch::combat_monster_attack_result_message(&state, attack)
            .expect("the theft narrates")
    ));
    assert_eq!(rows.message, "Pass\nA Gremlin stole some food!\n");
    assert_eq!(
        rows.message_entries()
            .iter()
            .map(|entry| entry.text.as_str())
            .collect::<Vec<_>>(),
        ["Pass", "A Gremlin stole some food!"]
    );
...
fn the_teleport_arm_spends_its_published_draw_budget() {
    assert!(combat_ai_teleport_chance_accepts(0));
    assert!(combat_ai_teleport_chance_accepts(1));
    assert!(combat_ai_teleport_chance_accepts(2));
    ...
    assert_eq!(combat_ai_teleport_probe_cell(0, 15), None);
    ...
    let accepted =
        combat_ai_teleport_chance_accepts(u5_prng_range_u16(&mut expected, 0, 3) as u8);
    if accepted {
...
fn the_reaper_and_the_mimic_are_refused_by_the_movement_arm() {
    ...
    assert!(!combat_ai_class_never_moves(COMBAT_CLASS_GIANT_RAT));
