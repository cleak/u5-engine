...
impl PlayState {
    ...
    pub fn resolve_and_apply_combat_targeting_attack(
        ...
        let (attacker_rating, defender_rating) =
            self.combat_player_attack_ratings(attacker_slot, target_slot)?;
        let inputs =
    ...
    /// The attacker and defender ratings one player-side attack feeds the
    /// shared to-hit helper. Shared by the readied-item and bare-handed arms
    /// so the two cannot drift apart.
    ///
    /// **Not `combat.md §11`'s selector.** `§11` publishes the attacker term
    /// as Strength only for the five strength-arm ids and "its own **combat
    /// weight**" otherwise - "Every other readied item, and bare hands,
    /// leaves the attacker term as Dexterity" (`catalogs/item-list.md §5.3`)
    /// - and the defender term as "**always** the combat weight". This is the
    /// engine's own pre-existing player-attack convention, kept because it is
    /// the pair the engine's held to-hit comparison was chosen against
    /// (`combat_actor::resolve_combat_hit`, blocked on `cleak/u5-spec#183`,
    /// whose answer `RETRACTIONS.md` R334/R335 have since published). Both
    /// arms therefore share one model rather than one arm conforming and the
    /// other not; re-deriving the model itself is a separate change.
    fn combat_player_attack_ratings(
        &self,
        attacker_slot: usize,
        target_slot: usize,
    ) -> Option<(u8, u8)> {
        let attacker_rating = self
            .combat_roster_slot_for_actor_slot(attacker_slot)
            .and_then(|roster_slot| self.party_strengths.get(roster_slot).copied())
            .unwrap_or(AVATAR_STAT_MAX);
        let defender_rating = if target_slot < COMBAT_PARTY_ACTOR_SLOTS {
            CHARACTER_DEFENSE_FACTORY_SEED
        } else {
        ...
        Some((attacker_rating, defender_rating))
    }
    ...
    ///
    /// It is an ordinary attempt, not a no-op. `§11`'s attacker-rating table
    /// carries the row "| Attacker | party member with any other readied
    /// item, or bare-handed | its own **combat weight** |", so the shared
    /// to-hit helper runs and takes its one draw; `§12`'s stage-one damage
    /// table says "Values `0` and `1` pass through unchanged, and **bare
    /// hands are a flat `1`**", so the damage stage takes no draw of its own.
    pub fn resolve_and_apply_combat_bare_handed_attack(
        ...
        let (attacker_rating, defender_rating) =
            self.combat_player_attack_ratings(attacker_slot, target_slot)?;
        let inputs = inputs.unwrap_or_else(|| self.combat_bare_handed_attack_inputs(attacker_slot));
        let resolution = resolve_combat_weapon_attack(
            COMBAT_BARE_HANDED_ATTACK_MAX,
            attacker.range_to(target),
            COMBAT_BARE_HANDED_RANGE_CAP,
            COMBAT_BARE_HANDED_EFFECT_CODE,
            attacker_rating,
            defender_rating,
            inputs.hit_roll,
            inputs.damage_roll,
    ...
    /// `combat.md §11`/`§12`: a bare-handed attempt draws the shared to-hit
    /// roll and nothing else - "**bare hands are a flat `1`**", and a flat
    /// stage-one value "passes through unchanged" without a damage draw.
    pub fn combat_bare_handed_attack_inputs(
        &mut self,
        attacker_slot: usize,
    ) -> CombatPlayerWeaponAttackInputs {
        let _ = attacker_slot;
            ...
            CombatTargetingCursorAction::Confirmed(cell) => {
                ...
                let attack = target_slot.and_then(|target_slot| {
                    ...
                    match item_id {
                        ...
                        None => self.resolve_and_apply_combat_bare_handed_attack(
                            session.actor_slot,
                            target_slot,
                            inputs,
                        ),
