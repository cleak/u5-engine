...
| `F` Fire | Ship cannon broadside | Mode-specific refusal | Mode-specific refusal | Combat fire command | `ship_fire` tests in `chunk_15` |
...
| `J` Jimmy | Lockpick/pocket/container routes where valid | Doors, chests, NPC pockets | Dungeon doors/chests | Combat scene branch | `active_dungeon_jimmy_picker_preserves_prompt_before_key_check` |
...
| `O` Open | Doors/containers where modeled | Doors, chests, locks, object helper | Underfoot dungeon doors/chests | Combat open command | `town_open_locked_sidecar_refuses_without_turn`, `dungeon_open_chest_consumes_turn_and_marks_visit_local_open_chest` |
| `P` Push | Static/dynamic pushables | Furniture, cannons, dynamic objects | Dungeon push semantics | Combat push command | `town_push_uses_clean_sidecar_to_swap_target_into_destination`, `world_push_static_family_wraps_and_advances_avatar` |
| `Q` Save / exit | Save prompt and save writer | Save prompt and save writer | Dungeon exit/save prompt handling | Combat quit/defeat path | `cli_binary_help_prints_usage_without_assets`, save/load tests in `chunk_03` |
...
| `X` X-it | Vehicle dismount | Vehicle/mode refusal | Mode-specific refusal | Combat escape/exit branch | `whirlpool_engagement_no_op_for_on_foot_party` and vehicle tests |
| `Y` Yell | Ship sails, words, Shadowlord names | Yell prompt and story words | Dungeon words | Combat command table owns combat behavior | `y_yell_shipboard_toggles_sails_and_non_ship_prompts_without_turn`, `y_yell_shadowlord_name_spawns_only_in_matching_virtue_town` |
