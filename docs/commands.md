...
| `E` Enter | Live tile selects exact narration; published 40-row coordinate table or class-complete sidecar selects destination | Mode-specific refusal/interaction | Dungeon-mode route as specified | Combat scene commands | `world_enter_all_published_locations_without_sidecar`, `world_enter_narration_and_current_plane_ool_precede_scene_load_failure` |
...
| `P` Push | Published static families; dynamic objects refuse | Published furniture/cannon families plus clean extensions; dynamic objects refuse | Exact `Push` / `Not here!` refusal | Shared static push/pull plus reveal preemption | `world_push_static_family_wraps_and_advances_avatar`, `push_prompt_ignores_escape_then_space_completes_pass_and_consumes_action`, `combat_push_reveal_marker_preempts_source_test_without_a_result_line` |
