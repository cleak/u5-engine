...
Last refreshed on 2026-05-19 during the Britannia chunk-map overlay visual and route-smoke audit.

| Area | Current status | Evidence | Remaining risk |
|---|---|---|---|
| Intro/menu | Terminal and Bevy intro shells use the runtime menu dispatcher; the Bevy title phase renders the spec-defined `TITLE.BIT`/`BRITISH.BIT` bitmap composition, drives an interruptible `BRITISH.PTH` signature-path animation, and advances a four-frame clean replacement title-tick strip before dismissal. The finished Bevy menu keeps the title surface alive while overlaying menu text and continuing the title tick; Bevy story slides compose the published transition-strip, doorway, and secondary art passes. Journey Onward, Create Character, and U4 transfer have runtime flows. | `intro_menu`, `menu_dispatch`, `chargen`, `u4_transfer`, runtime PTH parser tests, Bevy intro title/signature/tick/menu/story-draw tests, and `--visual-frame-suite` intro PNG/hash evidence in `TODO.md`. | Exact historical driver-resident title-tick pixels, exact story rectangle-transition wipe curve, visual polish, and full flow screenshot coverage. |
| World mode | Movement, vehicles, hazards, waterfalls, moongates, plane transitions, native and sidecar encounters, active objects, save/load, and many commands are implemented. Hoisted-sail movement follows the public wind wait-tick table, including the two-wait into-wind case. Natural moongate live-tile refresh and entry are implemented when callers seed the moon-glyph cache; otherwise entry refuses instead of guessing. | `cargo test -p u5-runtime`, world tests across chunks 03, 06, 07, 10, 12, 13, 15, 17, 23; focused sailing tests cover calm, perpendicular, with-wind, and into-wind hoisted-sail cadence; `cargo run -p u5-tui -- --route-smoke C:\Games\U5-Clean` passed 59 cases on 2026-05-19, including Spyglass Britannia chunk-map, H-Hole-up rest, utility U-Use items, dispatcher refusal routes, Shadowlord town-entry/Yell/Stonegate routes, and terminal endgame confirmations. | Exact public coordinate tables remain intentionally absent from the gazetteer; sidecars/debug entry cover those paths. Moon-glyph cache table semantics are blocked on public spec clarification in cleak/u5-spec#38. |
...
| Save/load | Known save fields, active objects, spell/reagent stock, transport markers, boarded ship hull/skiff state, overlays, dungeon working buffer, mirror files, wind-driven ship movement, parked vehicle board/exit/fire mutations, town floor transitions, dungeon level transitions, dungeon exit-to-world transitions, and world plane-transition overlay cache after save/load are covered. | Save/load tests in chunks 03, 04, 05, 07, 09, 12, 23. | Unknown byte preservation must be kept when adding new durable fields. |
| Rendering | TUI text/raster diagnostics, headless `--save-frame` PNG capture, `--save-frame-suite` PNG batches, Bevy atlas-backed views, route-smoke scripted hashes, Bevy `--visual-frame-suite` PNG batches, and a fixed-cell text-window runtime surface exist for world, town, dungeon, combat, intro, status, and modal panels. The TUI frame suite writes nonblank world, town, dungeon, combat, surface View, dungeon View, Peer, X-Ray, intro-menu, status-window, Z-stats-modal, and endgame-status PNGs. The Bevy visual frame suite writes seventeen nonblank composed PNGs for initial/stepped world, town, lit and dark dungeon, synthetic combat, surface/dungeon View overlays, Spyglass Britannia chunk-map overlay, Peer/X-Ray overlays, Z-stats modal, endgame status, title and finished-menu intro screens, transition-backed intro story art, and Return-to-View preview. The route-smoke suite now runs 59 asset-backed world/town/dungeon/combat/endgame cases including save prompts, look/pass flows, View overlays, Spyglass chunk-map overlay, H-Hole-up rest, utility U-Use items, world/town dispatcher refusal and party overlay routes, Shadowlord town-entry/Yell/Stonegate routes, terminal endgame confirmations, Peer/X-Ray overlays, Underworld startup, debug-enter return, seeded ship/skiff routes, dungeon exit prompts and A/S/G/J/O/refusal routing, Doom room combat trigger, and combat pass/A/C/G/J/O/P/K/R/Z/refusal/Look/View/Yell/X-it/Search-prompt command routing. Bevy gameplay status renders the shared text-window surface through the runtime-loaded `IBM.CH` font into a texture. | Raster hash, save-frame-suite, visual-frame-suite, and route-smoke notes in `TODO.md`; text-window and fixed-font renderer tests in chunks 13 and 14; `u5-tui` save-frame/play-loop/route-smoke tests; Bevy framebuffer and visual frame-suite tests for world, town, dungeon, combat, View/Peer/X-Ray/chunk-map overlays, intro title/signature/tick/menu/story art, status, and endgame modal surfaces. | Exact original modal rectangles, exact historical title-tick pixels, exact story rectangle-transition wipe curve, full UI composition parity, and a human-reviewed frontend screenshot gallery remain presentation work. |
