...
Last refreshed on 2026-08-27 in the current worktree at public spec head
`02550e2`, after reconciling all answered issues through `#177`. No clean-spec
issue remains open. Public
...
caller-composed state. Public `#168` retracted the phantom Blackthorn
`SAVED.BTH` state, and public `#169` fixes the cinematic presentation: slots
6/7 are the two guards, slot 8 is the suppressed/revealed seated tableau,
pause operands advance redraws without text output, explicit redraw never
clears the screen, and the rescue records all three exact cell reveals plus
the paired 1,856-draw thunder flashes. Public `#177` closes the audio boundary
too: every VM stinger uses the exact two-part 25+25-update recipe and rescue
runs the six independent envelope rows. Public issues `#170`–`#176` likewise
close and are implemented for CBT setup/exit metadata, tavern continuation
edges, combat factions, sailing collision/rough seas, wrong-Mix targeting,
moldy-corpse Search/Plague, and the controlled-party Vanish faint tail. No
clean-spec issue remains open. Public `#103` closed
...
| Save/load | Known save fields, active objects, spell/reagent stock, transport markers, boarded ship hull/skiff state, overlays, dungeon working buffer, mirror files, shared active-effect code/duration, and the exact queued-vehicle X/Y/packed-class bytes are covered. Queued shipwright delivery survives a save before town exit and successful delivery clears only its class byte. Save/load uses the typed disk-role session and shared retry wrapper; load refreshes the per-plane mirrors, while save reads `UNDER.OOL` then `BRIT.OOL`, builds canonical `SAVED.OOL`, conditionally rewrites unchanged `UNDER.OOL` according to the entry disk role, and never writes `BRIT.OOL`. Interactive launches detect the pristine/default or otherwise read-only install and run from a persistent writable local-data mirror, preserving both the clean source and the played save across Journey Onward and character creation. | Save/load tests in chunks 03, 04, 05, 07, 09, 11, 12, 13, 23, 25 plus `cli_binary_play_script_confirmed_save_round_trips_to_temp_save`; focused tests cover disk retries and role restoration, exact queued-vehicle preservation/increment/wrap/delivery, active-effect round trips, load mirror writes, save staging I/O counts, read-only runtime mirroring and save preservation, real Journey Onward/post-chargen launch smoke, and synthetic plus local-clean `.OOL` aggregate audits. | Unknown byte preservation must be kept when adding durable fields; uncommon `.OOL` auxiliary byte meanings outside published families remain catalog work, and exact historical floppy labels and swap timing remain presentation compatibility work. |
