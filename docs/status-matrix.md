...
Last refreshed on 2026-05-19 during the intro title/signature/tick/story-art and visual-frame audit.

| Area | Current status | Evidence | Remaining risk |
|---|---|---|---|
| Intro/menu | Terminal and Bevy intro shells use the runtime menu dispatcher; the Bevy title phase renders the spec-defined `TITLE.BIT`/`BRITISH.BIT` bitmap composition, drives an interruptible `BRITISH.PTH` signature-path animation, and advances a four-frame clean replacement title-tick strip before dismissal. Bevy story slides now compose the published transition-strip, doorway, and secondary art passes; Journey Onward, Create Character, and U4 transfer have runtime flows. | `intro_menu`, `menu_dispatch`, `chargen`, `u4_transfer`, runtime PTH parser tests, Bevy intro title/signature/tick/story-draw tests, and `--visual-frame-suite` intro PNG/hash evidence in `TODO.md`. | Exact historical driver-resident title-tick pixels, exact story rectangle-transition wipe curve, visual polish, and full flow screenshot coverage. |
