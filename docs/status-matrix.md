...
Last refreshed on 2026-05-19 during the intro title-art and visual-frame audit.

| Area | Current status | Evidence | Remaining risk |
|---|---|---|---|
| Intro/menu | Terminal and Bevy intro shells use the runtime menu dispatcher; the Bevy title phase renders the spec-defined `TITLE.BIT`/`BRITISH.BIT` bitmap composition before dismissal; Journey Onward, Create Character, and U4 transfer have runtime flows. | `intro_menu`, `menu_dispatch`, `chargen`, `u4_transfer`, Bevy intro title-art tests, and `--visual-frame-suite` `intro-menu` PNG/hash evidence in `TODO.md`. | `BRITISH.PTH` signature animation, visual polish, and full flow screenshot coverage. |
