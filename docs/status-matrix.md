...
Last refreshed on 2026-05-19 during the intro title/signature and visual-frame audit.

| Area | Current status | Evidence | Remaining risk |
|---|---|---|---|
| Intro/menu | Terminal and Bevy intro shells use the runtime menu dispatcher; the Bevy title phase renders the spec-defined `TITLE.BIT`/`BRITISH.BIT` bitmap composition and drives an interruptible `BRITISH.PTH` signature-path animation before dismissal; Journey Onward, Create Character, and U4 transfer have runtime flows. | `intro_menu`, `menu_dispatch`, `chargen`, `u4_transfer`, runtime PTH parser tests, Bevy intro title/signature tests, and `--visual-frame-suite` `intro-menu` PNG/hash evidence in `TODO.md`. | Title tick replacement art, visual polish, and full flow screenshot coverage. |
