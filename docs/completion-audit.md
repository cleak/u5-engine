...
| `u4-transfer.md` §1–§10 | `u4_transfer_preview.rs` (the single `PARTY.SAV` parser, `§5.1`/`§5.2`/`§5.3` per `cleak/u5-spec#88`), `u4_transfer.rs` commit side, `u4_transfer_session.rs` state machine, INIT.GAM/INIT.OOL seed handling, stat translation, OOL ordering | u4-transfer tests | Implemented |
