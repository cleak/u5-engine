...
| `traps.md` §1–§5 | `traps.rs::TrapEffect`, `containers.rs` trap routing; live dispatch now routes through `shared_trap_effect_family_from_index` rather than a hardcoded match | trap-effect tests, plus poison-helper and effect-distribution tests in `chunk_17.rs` | Implemented. **`§3`'s retraction fixed a serious live bug**: the status helper used by effect ids 1 and 3 is a *poison* primitive, not a revive primitive, and we had shipped the retracted draft verbatim (`if status == 'D' { status = 'P' }`). Poison traps therefore did **nothing at all** to a healthy party — 2/8 of non-combat trap rolls and *half* of every combat roll — and gas traps **resurrected dead members** instead of poisoning the living. Found by reference count, not by reading: `traps.rs` had 8 of 10 public functions with no production caller, and held both halves of the drift — five accurate derived predicates and three encoding the retracted contract, the latter pinned by a green test citing `§3`. Open gaps filed as `cleak/u5-spec#89`: the container trigger-slot rule is unpublished (our active-player fallback is invented and marked as such), and `§3` is silent on whether Ashes shares the Dead exclusion |
